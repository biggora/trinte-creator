 ...
 */
exports.execute = function (params, appPath, options, appLauncher) {
    ...
    if (controllerName !== options.model.capitalize()) {
        console.log("Using Controller name as plural not singular: " + controllerName);
    }
    ...
    if (options.action !== "controller") {
        ...
        controllerTemplate = __dirname + '/../scripts/controllers/Controller.ejs';
        ...
        controllerTemplate = __dirname + '/../scripts/controllers/EmptyController.ejs';
    }
    ...
    if (fs.existsSync(appPath + '/.trinterc')) {
        var pd = fs.readFileSync(appPath + '/.trinterc', "utf-8");
        try {
    ...
    // Write the files
    if (options.action === "controller") {
        var viewEngine = projectdata.engine;
        var viewTemplates = require('trinte-' + viewEngine);
        var viewEmptyTemplate = viewTemplates.getTemplate('empty');
