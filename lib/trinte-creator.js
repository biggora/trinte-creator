...
exports.createScript = function createScript(appLauncher) {
    ...
    var params = appLauncher.script.params || [], options = {};
...
exports.buildAppStructure = function buildAppStructure(appLauncher, callback) {
    ...
    if (appLauncher.auth === true) {
        ...
        exports.writeModelFile(appLauncher.bootstrapPath + '/app/snippets/routes-auth.ejs', appLauncher.projectPath + '/config/routes.' + appLauncher.js, appLauncher);
        ...
        exports.writeModelFile(appLauncher.bootstrapPath + '/app/snippets/routes.ejs', appLauncher.projectPath + '/config/routes.' + appLauncher.js, appLauncher);
    }
    ...
    process.nextTick(function() {
        fs.writeFileSync(targetViews + '/_layout' + viewEngine.extension, parser.render(fs.readFileSync(coreView + '/main/_layout' + viewEngine.extension, 'utf8'), vloc), 'utf8');
        fs.writeFileSync(targetViews + '/_header' + viewEngine.extension, parser.render(fs.readFileSync(coreView + '/main/_header' + viewEngine.extension, 'utf8'), vloc), 'utf8');
        fs.writeFileSync(targetViews + '/_footer' + viewEngine.extension, parser.render(fs.readFileSync(coreView + '/main/_footer' + viewEngine.extension, 'utf8'), vloc), 'utf8');
        fs.writeFileSync(targetViews + '/_messages' + viewEngine.extension, parser.render(fs.readFileSync(coreView + '/main/_messages' + viewEngine.extension, 'utf8'), vloc), 'utf8');
        fs.writeFileSync(targetViews + '/login' + viewEngine.extension, parser.render(fs.readFileSync(coreView + '/main/login' + viewEngine.extension, 'utf8'), vloc), 'utf8');
    });
    exports.createDir(targetViews + '/app');
    exports.copyFiles(coreView + '/main/app', targetViews + '/app', new RegExp('.' + viewEngine.extension + "$"));
    ...
    var dloc = {
        locals: {
            ...
            username: aDb[3] || "",
            password: aDb[4] || "",
            database: aDb[1] || "trinte-dev"
        },
        open: "<?",
        close: "?>"
    };
    if (dloc.locals.driver === 'sqlite3') {
        if (!/\//.test(dloc.locals.database)) {
            exports.createDir(rootPath + '/db');
            dloc.locals.database = './db/' + dloc.locals.database;
        }
    }
...
exports.createPackageInfo = function createPackageInfo(appLauncher) {
    ...
    for (var key in appLauncher) {
        if (/^engine$|^css$|^js$|^theme$|^socket$|^auth$|^session$/i.test(key)) {
    ...
    if (appLauncher.auth === true) {
        json.dependencies["passport"] = ">=0.1.18";
        json.dependencies["passport-local"] = ">=0.1.6";
    ...
    if (appLauncher.session === true) {
        json.dependencies["connect-caminte"] = "0.0.x";
    }
