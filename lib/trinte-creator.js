            ...
            util.question('Enter Project version (' + appLauncher.version + ')>', function (version) {
                ...
                util.question('Enter Project license (' + appLauncher.license + ')>', function (license) {
                    ...
                    exports.buildAppStructure(appLauncher, function () {
                        logger.verbose('\nApplication created!');
                        logger.notice('\nNext step run the app:'.bold);
                        logger.notice(('cd ' + appLauncher.projectPath + ' && trinte s').bold);
                    });
                });
            });
        });
...
exports.createScript = function createScript(appLauncher) {
    ...
    options.projectInfo = util.getBootstrapData(appLauncher.projectPath);
...
exports.buildAppStructure = function buildAppStructure(appLauncher, callback) {
    ...
    exports.copyFiles(__dirname + '/../app/core/', rootPath, new RegExp(filter + "|.md$"));
    exports.createDir(rootPath + '/bin');
    exports.copyFiles(__dirname + '/../app/bin/', rootPath + '/bin', new RegExp(filter));
    exports.createDir(rootPath + '/config');
    exports.copyFiles(__dirname + '/../app/config/', rootPath + '/config', new RegExp(filter + "|.yml$"));
    ...
    if (appLauncher.session === true) {
        exports.writeModelFile(__dirname + '/../app/snippets/session-store.ejs', appLauncher.projectPath + '/config/session.' + appLauncher.js, appLauncher);
        // exports.writeModelFile(scriptsPath + '/models/Session.ejs', rootPath + '/app/models/Session.' + appLauncher.js, appLauncher);
    } else {
        exports.writeModelFile(__dirname + '/../app/snippets/session.ejs', appLauncher.projectPath + '/config/session.' + appLauncher.js, appLauncher);
    }

    if (appLauncher.auth === true) {
        exports.createDir(rootPath + '/config/authorization');
        exports.writeModelFile(__dirname + '/../app/snippets/local-auth.ejs', appLauncher.projectPath + '/config/authorization/local.' + appLauncher.js, appLauncher);
        exports.writeModelFile(__dirname + '/../app/snippets/middleware-auth.ejs', appLauncher.projectPath + '/config/middleware.' + appLauncher.js, appLauncher);
        ...
        exports.writeModelFile(__dirname + '/../app/snippets/middleware.ejs', appLauncher.projectPath + '/config/middleware.' + appLauncher.js, appLauncher);
    }

    if (appLauncher.mailer === true) {
        exports.createDir(rootPath + '/config/addons');
        exports.writeModelFile(__dirname + '/../app/snippets/mailer.ejs', appLauncher.projectPath + '/config/addons/mailer.' + appLauncher.js, appLauncher);
        exports.writeModelFile(__dirname + '/../app/snippets/mail.ejs', appLauncher.projectPath + '/config/mail.' + appLauncher.js, appLauncher);
    ...
    if (appLauncher.uploader === true) {
        exports.createDir(rootPath + '/config/addons');
        exports.writeModelFile(__dirname + '/../app/snippets/uploader.ejs', appLauncher.projectPath + '/config/addons/uploader.' + appLauncher.js, appLauncher);
    ...
    if (appLauncher.cart === true) {
        exports.createDir(rootPath + '/config/addons');
        exports.writeModelFile(__dirname + '/../app/snippets/cart.ejs', appLauncher.projectPath + '/config/addons/cart.' + appLauncher.js, appLauncher);
    ...
    if (appLauncher.recaptcha === true) {
        exports.createDir(rootPath + '/config/addons');
        exports.writeModelFile(__dirname + '/../app/snippets/recaptcha.ejs', appLauncher.projectPath + '/config/addons/recaptcha.' + appLauncher.js, appLauncher);
    ...
    var routesFile = fs.readFileSync(__dirname + '/../app/snippets/routes.ejs', 'utf8');
    routesFile = routesFile
        .replace('{ADDONSINC}', rAddons.inc.join('\n'))
        .replace('{ADDONSBODY}', rAddons.body.join('\n'));
    fs.writeFileSync(__dirname + '/../app/config/routes.' + appLauncher.js, routesFile, 'utf8');
    ...
    fs.writeFileSync(rootPath + '/config/database.' + appLauncher.js, parser.render(fs.readFileSync(__dirname + '/../app/snippets/database.ejs', 'utf8'), dloc), 'utf8');
    ...
    logger.status('#   create ./' + relPath + '/package.json');
    exports.createTrinteRC(appLauncher);
    logger.status('#   create ./' + relPath + '/.trinterc');
    util.writeJSON(rootPath + '/bower.json', {
        name: appLauncher.name,
        description: appLauncher.description,
        author: appLauncher.author,
        version: appLauncher.version,
        license: appLauncher.license,
        dependencies: {
            'jquery': '^2.2.1',
            'html5shiv': '^3.7.3',
            'respond': '^1.4.2'
    ...
    // 'bootstrap': '^3.3.6',
    // 'font-awesome': '^4.4.0',
    logger.status('#   create ./' + relPath + '/bower.json');
    util.writeJSON(rootPath + '/.bowerrc', {
        "directory": "public/vendor"
    });
    logger.status('#   create ./' + relPath + '/.bowerrc');
    util.writeJSON(rootPath + '/.jshintrc', {
        "node": true,
        "esnext": true,
        "bitwise": true,
        "camelcase": false,
    ...
    // tests
    exports.createDir(rootPath + '/test/controllers');
    exports.createDir(rootPath + '/test/models');
    exports.createDir(rootPath + '/test/units');
    exports.writeModelFile(__dirname + '/../app/snippets/tests.js', appLauncher.projectPath + '/test/test.' + appLauncher.js, appLauncher);

    // gulp tasks
    exports.copyFiles(__dirname + '/../scripts/gulp', appLauncher.projectPath + '/gulp', new RegExp('\.js$'));

    logger.status('#   create ./' + relPath + '/.jshintrc');
    logger.notice('#   Install dependencies via npm');
    process.nextTick(function () {
        exports.npmInstall(rootPath, function () {
            logger.notice('#   Install dependencies via bower');
            exports.bowerInstall(rootPath, function () {
                callback && callback();
            });
        });
    });
...
exports.createPackageInfo = function createPackageInfo(appLauncher) {
    var json = exports.readPackageInfo(__dirname + '/../app/package.json');
    ...
    if (appLauncher.auth === true) {
        ...
        json.dependencies["trinte-auth"] = "latest";
    }

    if (appLauncher.uploader === true) {
        json.dependencies["express-uploader"] = "latest";
...
/**
 * Create file .trinterc.
 *
 * @param {Object} appLauncher
 **/
exports.createTrinteRC = function createTrinteRC(appLauncher) {
    var json = {}, targetFile = appLauncher.projectPath + '/.trinterc';
    json.name = appLauncher.name;
    if (appLauncher.description !== "") {
        json.description = appLauncher.description;
    }
...
exports.copyLicense = function copyLicense(appLauncher) {
    ...
    try {
        if (fs.existsSync(__dirname + '/../app/LICENSES/' + license)) {
            licenseSource = __dirname + '/../app/LICENSES/' + license;
        }
    } catch (err) {
        licenseSource = __dirname + '/../app/LICENSES/MIT';
    }
 ...
 * @param {Function} cb
 */
exports.createGulpTasks = function createGulpTasks(dstpath, cb) {
    exports.copyFiles(from, to, filter)
 ...
 * @param {Function} cb
 */
exports.npmInstall = function npmInstall(dstpath, cb) {
    process.chdir(dstpath);
    exec('npm install --save --save-dev',
...
/**
 * Change dir & bower install
 *
 * @param {String} dstpath
 * @param {Function} cb
 */
exports.bowerInstall = function bowerInstall(dstpath, cb) {
    process.chdir(dstpath);
    exec('bower install --allow-root',
        function (error, stdout, stderr) {
            if (error !== null) {
                console.log(error);
                if (/command\snot\sfound/i.test(stdout)) {
                    logger.notice('Please first install bower');
                    logger.warning('npm install bower -g');
                    logger.notice('then install dependencies');
                    logger.warning('cd ' + dstpath + ' && bower install --allow-root');
                    logger.notice('.');
                }
            } else {
                logger.notice('#   Complete!');
