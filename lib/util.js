...
util.initParams = function(params, i) {
    ...
    if (i === 2) {
        util.appLauncher.command = (params[i] || "").replace(/^-+/, "");
        ...
        } else if (/init|create-app/.test(util.appLauncher.command)) {
            util.appLauncher.command = 'init';
            var appname = (params[1 + i] || "").replace(/^\s+|\s+$/, "");
            if (appname !== '') {
                util.appLauncher.app = appname;
            } else {
                util.abort("Invalid Application name!");
            }
    ...
    if (/ejs|jade|jshtml|hogan|haml/.test(fparam)) {
        util.appLauncher.engine = fparam;
    }

    if (/css|less|sass|tylus/.test(fparam)) {
        util.appLauncher.css = fparam;
    }

    if (/js|coffee/.test(fparam)) {
        util.appLauncher.js = fparam;
    }

    if (/theme/.test(fparam) && params[1 + i] !== 'undefined') {
        util.appLauncher.theme = params[1 + i];
    }

    if (/session|sess/.test(fparam)) {
    ...
    if ((util.appLauncher.command === "g" || util.appLauncher.command === "script" || util.appLauncher.command === "test") && i === 3) {
        util.appLauncher.script.name = param;
    }
